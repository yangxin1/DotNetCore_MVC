...
namespace DAL
{
    ...
    public class DBHelper<T> : IDBHelper<T>
    {
        ...
        /// 连接字符串
        /// </summary>
        public MySqlConnection _conn => new MySqlConnection("Data Source=127.0.0.1;database=school;User ID=root;Password=831143;");//使用表达式树设置属性
        //使用get属性
        //{
        //    get
        //    {
        //        return new MySqlConnection("Data Source=127.0.0.1;database=school;User ID=root;Password=831143;");
        //    }
        //}
        ...
        public virtual async Task<List<T>> GetList(string Tbname, int pageNO = 1, int pagecount = 10)
        {
            ...
            try
            {
                resultList = await Task.Run(() =>
                {
                    ...
                    //    resultList.Add(q);
                    //}
                    resultList = query.AsList();//使用AsList转换为List
                    if (resultList == null)
                    {
        ...
        /// <returns></returns>
        public async Task<T> ExecuteToModel(string sql)
        {
            T result = default(T);
            try
            {
        ...
        /// <returns></returns>
        public async Task<string> MD5encrypt16(string password)
        {
            if (password == null) return "";
            var MD5 = new MD5CryptoServiceProvider();
            return await Task.Run(() =>
            {
                string MDpassword = BitConverter.ToString(MD5.ComputeHash(Encoding.Default.GetBytes(password)), 4, 8);
                MDpassword = MDpassword.Replace("-", "");
                return MDpassword;
            });
        }
        #endregion
